 ...
 * 0x88A8
 */
static int mvpp2_prs_vlan_init(struct malloc_type *malltype, struct mvpp2 *priv)
{
	struct mvpp2_prs_entry pe;
	int err;

	priv->prs_double_vlans = mallocarray(sizeof(bool), MVPP2_PRS_DBL_VLANS_MAX, malltype, M_WAITOK | M_ZERO);
...
int mvpp2_prs_default_init(struct malloc_type *malltype, struct mvpp2 *priv)
{
	int err, index, i;
	...
	priv->prs_shadow = mallocarray(MVPP2_PRS_TCAM_SRAM_SIZE, sizeof(*priv->prs_shadow), malltype, M_WAITOK | M_ZERO);
	if (!priv->prs_shadow)
		return -ENOMEM;
	...
	err = mvpp2_prs_vlan_init(malltype, priv);
	if (err)
		return err;
...
int mvpp2_prs_mac_da_accept(struct mvpp2_port *port, const u8 *da, bool add)
{
	...
	if (is_broadcast_ether_addr(da)) {
		...
		//XXX if (ether_addr_equal(da, port->dev->dev_addr))
...
int mvpp2_prs_update_mac_da(struct net_device *dev, const u8 *da)
{
	...
	//XXX err = mvpp2_prs_mac_da_accept(port, dev->dev_addr, false);
	//XXX if (err)
	//XXX 	return err;
	...
	//XXX ether_addr_copy(dev->dev_addr, da);
...
void mvpp2_prs_mac_del_all(struct mvpp2_port *port)
{
	...
	for (tid = MVPP2_PE_MAC_RANGE_START;
		 ...
		 */
		if (is_broadcast_ether_addr(da) ||
			 false)//XXX ether_addr_equal(da, port->dev->dev_addr))
			continue;
